...
import ContactFormFloorPlan from "./FloorPlanForm";
      ...
      {props?.isOpen && (
        ...
        >
          <div className="relative w-11/12 max-w-md p-6 bg-white rounded-lg shadow-lg">
            <h2 className="mb-4 text-xl font-semibold text-center text-black">
              {props?.title}
            </h2>
            {/* <p class="text-center mb-4 lg:mb-6 text-black">
              Please fill out the form
            </p> */}
            <ContactFormFloorPlan
              title={props?.title}
              source={props?.source}
            //   projectEnquire={props?.projectEnquire}
              hidemsg
              onSub={props.onSubmit}
            />
            <button
